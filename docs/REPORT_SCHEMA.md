...
- `selection_dossier.verdict.label`：公开选题级别，只允许 `推荐` 或 `不入池`。推荐表示可以进入写作框架；不入池表示证据、冲突、读者收益、重复度或传播张力不够，不再作为简讯或可选线索输出。
...
- `source_type`：来源抓取方式。`api-free` 表示免费 API 元数据源，例如 TopHubData 免费榜单节点列表；这类来源不等于热点事实本身，不能作为热点内容证据，写作前仍需补可复查的一手或近源证据。
