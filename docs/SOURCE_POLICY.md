...
TopHubData 使用边界：默认只调用免费接口。官方成本表显示“全部榜单列表”和“单个榜单快照列表”为免费；但快照列表只返回快照 ID/时间戳，不返回热点标题。“单个榜单最新详细”“全网热点内容搜索”“今日热榜榜中榜”“单个榜单快照详情”“热点日历事件”均会消耗 u。当前实现只允许在 `TOPHUBDATA_ENABLE_PAID_DETAIL=true` 时调用“单个榜单最新详细”，并用 `TOPHUBDATA_DETAIL_LIMIT_PER_RUN` 控制每轮最多调用节点数。
