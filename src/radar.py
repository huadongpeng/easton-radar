...
ENTITY_COOLDOWN_DAYS = 5
ENTITY_TOPIC_COOLDOWN_DAYS = 14
...
ENTITY_ALIASES = {
    "anthropic": ["anthropic", "claude", "fable", "mythos", "a 社", "a社"],
    "openai": ["openai", "chatgpt", "codex", "gpt"],
    "google": ["google", "gemini", "deepmind", "ai overviews", "谷歌"],
    "github": ["github", "copilot", "actions", "npm"],
    "apple": ["apple", "siri", "wwdc", "苹果"],
...
NARRATIVE_ANGLES = {
    "access_policy": ["封", "下架", "暂停", "限制", "访问", "政府", "指令", "directive", "access", "suspend", "ban", "blocked", "管制", "封号"],
    "trust_safety": ["安全", "信任", "漏洞", "警告", "反噬", "风险", "隐性", "不透明", "trust", "safety", "warning", "risk"],
    "cost_pricing": ["价格", "成本", "账单", "token", "额度", "降价", "涨价", "计费", "pricing", "cost", "billing", "credits"],
    "developer_workflow": ["编程", "开发", "agent", "workflow", "codex", "cursor", "ide", "代码", "效率", "工具链"],
    "side_business": ["副业", "独立开发", "外包", "变现", "用户", "破解", "库存", "现金流", "赚钱", "收入"],
    "platform_rules": ["规则", "协议", "合规", "账号", "搜索", "流量", "推荐", "平台", "policy", "terms", "compliance"],
}
...
def canonical_entity(text: str, url: str = "") -> str:
    haystack = f"{text} {url}".lower()
    for entity, aliases in ENTITY_ALIASES.items():
        if any(alias.lower() in haystack for alias in aliases):
            return entity
    # Do not fall back to the article host. Media/source hosts such as TechCrunch
    # or Simon Willison are not topic entities; treating them as entities blocks
    # unrelated stories from the same feed during busy news cycles.
    return ""


def narrative_angle(text: str) -> str:
    haystack = text.lower()
    scores = {
        angle: sum(1 for keyword in keywords if keyword.lower() in haystack)
        for angle, keywords in NARRATIVE_ANGLES.items()
    }
    angle, score = max(scores.items(), key=lambda item: item[1])
    return angle if score > 0 else "general"


def topic_cluster_key(title: str, original_title: str = "", url: str = "", topic_direction: str = "") -> str:
    text = f"{title} {original_title}"
    entity = canonical_entity(text, url)
    if not entity:
        return ""
    return "|".join(part for part in [entity, topic_direction, narrative_angle(text)] if part)
...
def decision_topic_cluster(decision: "RadarDecision", site: dict[str, Any]) -> str:
    topic_key, _ = topic_direction_for_item(decision.item, decision.report_type, site)
    return topic_cluster_key(decision.report_title or "", decision.item.title or "", decision.item.url, topic_key)
...
def archive_topic_cluster(item: dict[str, Any]) -> str:
    existing = str(item.get("topic_cluster", "")).strip()
    if existing:
        return existing
    return topic_cluster_key(
        str(item.get("title", "")),
        str(item.get("original_title", "")),
        str(item.get("url", "")),
        str(item.get("topic_direction", "")),
    )


def topic_cluster_entity(cluster: str) -> str:
    return cluster.split("|", 1)[0] if cluster else ""


def specific_topic_terms(title: str, original_title: str = "") -> set[str]:
    text = f"{title} {original_title}".lower()
    return {term for term in SPECIFIC_TOPIC_TERMS if term in text}
...
def rebuild_topic_archive_from_reports() -> dict[str, Any]:
    ...
    for path in sorted(REPORTS_DIR.glob("*.json"), key=lambda p: p.stat().st_mtime, reverse=True)[:240]:
        ...
        items.append({
            ...
            "url": report.get("url", ""),
            **fingerprints,
            "topic_cluster": topic_cluster_key(report.get("title") or "", report.get("original_title", ""), report.get("url", ""), report.get("topic_direction", "")),
...
def evidence_rank(value: str) -> int:
    return {"weak": 0, "media": 1, "near_source": 2, "official": 3}.get(str(value or ""), 0)


def can_upgrade_optional_topic(decision: "RadarDecision", item: dict[str, Any], site: dict[str, Any], is_today: bool = False) -> bool:
    if is_today or archive_item_level(item) != "可选":
        return False
    current_evidence = evidence_rank(decision.evidence_level)
    archived_evidence = evidence_rank(str(item.get("evidence_level", "")))
    archived_score = int(item.get("score", 0) or 0)
    topic_key, _ = topic_direction_for_item(decision.item, decision.report_type, site)
    ...
    if current_evidence > archived_evidence and decision.score >= 68 and tension.get("score", 0) >= 7:
        return True
    return decision.score >= archived_score + 10 and decision.score >= 68 and tension.get("score", 0) >= 7


def duplicate_or_optional_upgrade(decision: "RadarDecision", site: dict[str, Any], item: dict[str, Any], reason: str, is_today: bool = False) -> tuple[bool, str]:
    if can_upgrade_optional_topic(decision, item, site, is_today=is_today):
        return False, ""
    return True, reason
...
def is_duplicate_topic(decision: "RadarDecision", site: dict[str, Any], archive: dict[str, Any], batch_id: str, days: int = DUPLICATE_LOOKBACK_DAYS) -> tuple[bool, str]:
    ...
    cluster = decision_topic_cluster(decision, site)
    entity = topic_cluster_entity(cluster)
    for item in today_archive_items(archive):
        if item.get("url") and normalize_url(str(item.get("url"))) == url:
            return duplicate_or_optional_upgrade(decision, site, item, f"今日已收录同 URL：{item.get('title', '')}", is_today=True)
        if any_similar_fingerprint(archive_fingerprints(item), fingerprints, threshold=0.62):
            return duplicate_or_optional_upgrade(decision, site, item, f"今日已收录相似选题：{item.get('title', '')}", is_today=True)
        if cluster and archive_topic_cluster(item) == cluster:
            return duplicate_or_optional_upgrade(decision, site, item, f"今日已收录同主题簇：{item.get('title', '')}", is_today=True)
    generic_reason = same_host_recent_generic_topic(decision, site, archive)
    if generic_reason:
        return True, generic_reason
    decision_specific_terms = specific_topic_terms(decision.report_title, decision.item.title)
    if entity and decision_specific_terms:
        for item in recent_archive_items(archive, ENTITY_TOPIC_COOLDOWN_DAYS):
            if topic_cluster_entity(archive_topic_cluster(item)) != entity:
                continue
            item_terms = specific_topic_terms(str(item.get("title", "")), str(item.get("original_title", "")))
            overlap = sorted(decision_specific_terms & item_terms)
            if overlap:
                return duplicate_or_optional_upgrade(decision, site, item, f"近 {ENTITY_TOPIC_COOLDOWN_DAYS} 天同主体/同产品已收录（{', '.join(overlap)}）：{item.get('title', '')}")
    for item in recent_archive_items(archive, ENTITY_TOPIC_COOLDOWN_DAYS):
        if cluster and archive_topic_cluster(item) == cluster:
            return duplicate_or_optional_upgrade(decision, site, item, f"近 {ENTITY_TOPIC_COOLDOWN_DAYS} 天同主体/同角度已收录：{item.get('title', '')}")
    for item in recent_archive_items(archive, days):
        if item.get("url") and normalize_url(str(item.get("url"))) == url:
            return duplicate_or_optional_upgrade(decision, site, item, f"近 {days} 天已收录同 URL：{item.get('title', '')}")
        if any_similar_fingerprint(archive_fingerprints(item), fingerprints):
            return duplicate_or_optional_upgrade(decision, site, item, f"近 {days} 天已收录相似选题：{item.get('title', '')}")
    return False, ""
...
def archive_entry(report: dict[str, Any], batch: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        **report_fingerprints(display_report_title(report), report.get("original_title", "")),
        "topic_cluster": topic_cluster_key(display_report_title(report), report.get("original_title", ""), report.get("url", ""), report.get("topic_direction", "")),
        "topic_direction": report.get("topic_direction", ""),
        "topic_direction_title": report.get("topic_direction_title", ""),
...
def legacy_deepseek_triage_batch(items: list[SourceItem], site: dict[str, Any], policy: dict[str, Any]) -> list[RadarDecision]:
    ...
    body = {
        ...
        "请只输出 JSON：{\"items\":[...]}。每项包含 id, decision(deep_dive|skip), report_type, report_title"
...
def topic_level(report: dict[str, Any], dossier: dict[str, Any] | None = None) -> str:
    ...
    if label in {"不入池", "跳过"}:
        return "不入池"
    if label in {"最推荐", "强推荐", "推荐", "次推荐", "可选"}:
        if label in {"最推荐", "强推荐", "推荐", "次推荐"} and tension.get("score", 0) < 7:
            return "可选"
        return "推荐" if label in {"最推荐", "强推荐", "推荐", "次推荐"} else "可选"
    if (
        decision == "deep_dive"
...
def normalize_selection_dossier(report: dict[str, Any], dossier: dict[str, Any]) -> dict[str, Any]:
    ...
    if not verdict.get("reason"):
        if level == "推荐":
            verdict["reason"] = "证据、人设解读角度、目标读者兴趣和传播张力较完整，建议进入写作框架。"
        elif level == "可选":
            verdict["reason"] = "方向有价值，但证据、传播张力或写作闭环还不够完整，先进入可选池等待补证。"
        else:
            verdict["reason"] = "未达到公众号选题标准，不进入推荐或可选池。"
...
def enforce_evidence_gate(dossier: dict[str, Any], evidence: list[dict[str, str]]) -> dict[str, Any]:
    ...
    verdict["status"] = "不入池"
    verdict["label"] = "不入池"
    verdict["reason"] = "补证搜索没有拿到可用结果，不进入公众号主选题。"
...
def apply_quality_gate(report: dict[str, Any], dossier: dict[str, Any], evidence: list[dict[str, str]]) -> dict[str, Any]:
    ...
    if not isinstance(data, dict):
        ...
        verdict["status"] = "不入池"
        verdict["label"] = "不入池"
        verdict["reason"] = "质量闸调用失败，不进入公众号主选题。"
        return dossier
    ...
    verdict = dossier.setdefault("verdict", {})
    if recommendation in {"hold", "downgrade_to_brief", "optional"} and not data.get("fatal_issues"):
        verdict["status"] = "可选选题"
        verdict["label"] = "可选"
    else:
        verdict["status"] = "不入池"
    ...
    if isinstance(issues, list) and issues:
        ...
        verdict["reason"] = "质量闸没有给出推荐结论，先进入可选池等待补证。" if verdict["label"] == "可选" else "质量闸没有给出推荐或可选结论，不进入选题池。"
...
def compose_topic_dossier(report: dict[str, Any], site: dict[str, Any], policy: dict[str, Any], plan: dict[str, Any], evidence: list[dict[str, str]], previous: dict[str, Any] | None = None) -> dict[str, Any] | None:
    prompt = (
        ...
        "只有证据较完整、老花人设解读角度清楚、目标读者分层明确、泛兴趣故事钩子不夸张、传播张力成立且逻辑可闭环时才写推荐；方向有价值但还需要补证、补基础概念、补反方材料或补传播张力时写可选；证据不足、缺少真实冲突点、缺少讨论空间、同类主题近期重复或只能写成简讯时写不入池，并把 additional_search_queries 和缺口写清楚。"
    )
    data = deepseek_json(
        [
            {"role": "system", "content": prompt},
...
def selection_verdict(report: dict[str, Any]) -> dict[str, str]:
    ...
    if decision == "deep_dive" and evidence in {"official", "near_source"} and score >= 68 and gaps <= 3 and tension["score"] >= 7:
        return {
            "status": "推荐选题",
            "label": "推荐",
            "reason": "当前线索有明确来源、人设解读角度、目标读者兴趣、传播张力和分析空间，可以进入写作框架。",
        }
    return {
        "status": "可选选题" if decision == "deep_dive" and score >= MIN_DEEP_DIVE_SCORE else "不入池",
        "label": "可选" if decision == "deep_dive" and score >= MIN_DEEP_DIVE_SCORE else "不入池",
        "reason": "当前材料有选题价值，但证据、传播张力或逻辑闭环还不够完整，先进入可选池等待补证。" if decision == "deep_dive" and score >= MIN_DEEP_DIVE_SCORE else "当前材料未达到公众号选题标准，不进入推荐或可选池。",
    }
...
def selection_dossier(report: dict[str, Any]) -> dict[str, Any]:
    ...
    closure = [
        ...
        {
            "node": "材料是否足够写正文",
            "status": "建议扩写" if verdict["label"] == "推荐" else ("可选，需补证" if verdict["label"] == "可选" else "不入池"),
    ...
    return {
        ...
        "human_judgment_path": [
            ...
            "最后决定：推荐、可选或不入池，而不是把所有线索都写成同一套报告。",
        ],
...
def select_reports(decisions: list[RadarDecision], site: dict[str, Any], archive: dict[str, Any], batch_id: str, limit: int = MAX_REPORTS_PER_BATCH) -> tuple[list[RadarDecision], list[dict[str, str]]]:
    ...
    def try_select(decision: RadarDecision) -> bool:
        ...
        if any_similar_fingerprint(seen_fingerprints, fingerprints):
            ...
            return False
        cluster = decision_topic_cluster(decision, site)
        if cluster and any(decision_topic_cluster(selected_decision, site) == cluster for selected_decision in selected):
            duplicate_skips.append({"title": decision.report_title or decision.item.title, "url": decision.item.url, "reason": "本批次同主题簇已入池"})
            info(
                "Selection filter: "
                f"stage=batch-topic-cluster, score={decision.score}, decision={decision.decision}, topic={topic_key}, "
                f"source={decision.item.source_name}/{decision.item.source_type}, title={decision.report_title}, reason=本批次同主题簇已入池"
            )
            return False
...
def render_item(report: dict[str, Any], site: dict[str, Any]) -> str:
    ...
    verdict_status = "推荐选题" if label == "推荐" else ("可选选题" if label == "可选" else "不入池")
