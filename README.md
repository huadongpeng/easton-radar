...
- 给 GPT 编辑应用：来源、事实、时间线、证据地图、分析维度、标题种子、开头切入素材、存疑点、可展开角度、不可写成结论的点、需要补证的问题。
- 给 CMS：slug、canonical URL、SEO 标题、摘要、标签、选题方向、报告类型、来源分类、证据等级、资料包、发布状态。
...
报告类型只表示分析方法，不承担主栏目职责，也不生成公开栏目页。公开网站只按 `topic_direction` 组织，避免后续 GPT 应用和 CMS 被“分析方法”误导。
...
稳定信息源
  ...
  -> 判断 topic_direction
  -> 判断 report_type
  -> 生成简报、调查报告 JSON、material_pack 和 downstream_handoff
  -> 生成静态网站
  -> GitHub Pages 发布
  -> Telegram 通知
```
